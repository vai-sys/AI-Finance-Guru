...
const createTransaction = async (req, res) => {
  try {
    ...
    if (category_id) {
      ...
      // keep as string id; mongoose will cast when saving
      categoryObjectId = String(category_id);
    ...
    // populate category so frontend can show category name immediately
    await transaction.populate({ path: 'category_id', select: '_id name' });
  ...
  try {
    ...
    const [total, transactions] = await Promise.all([
      ...
      Transaction.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .populate({ path: 'category_id', select: '_id name type' })
        .lean()
    ]);
...
const updateTransaction = async (req, res) => {
  try {
    ...
    for (const key of allowed) {
      if (req.body[key] !== undefined) {
        ...
        if (key === 'category_id') {
          ...
          // assign id string -> mongoose will cast on save
          tx.category_id = String(cid);
          continue;
        }
    ...
    // populate category for response so frontend sees name
    await tx.populate({ path: 'category_id', select: '_id name' });
