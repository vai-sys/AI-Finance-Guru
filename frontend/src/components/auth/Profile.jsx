...
const Profile = () => {
    ...
    return (
      <div className="min-h-screen bg-[#F3F4F6] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#6B7280] font-medium">Loading your profile...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] relative overflow-hidden">
      {/* Background decoration */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-[#D1FAE5] rounded-full blur-3xl opacity-30"></div>
        <div className="absolute top-1/2 -left-40 w-80 h-80 bg-[#4ADE80] rounded-full blur-3xl opacity-20"></div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 relative z-10">
        ...
        <div className="flex items-center justify-between mb-8 pt-4">
          <div>
            <h1 className="text-4xl font-bold text-[#0F172A] tracking-tight">Profile</h1>
            <p className="text-[#6B7280] mt-1">Manage your personal information</p>
          </div>
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-[#0F172A] hover:text-[#EF4444] bg-white hover:bg-[#D1FAE5] border border-[#6B7280]/20 rounded-xl transition-all duration-200 shadow-sm hover:shadow-md"
          >
            <LogOut className="w-4 h-4" />
            <span className="hidden sm:inline">Sign out</span>
          </button>
        ...
        {/* Success Message */}
        {message && (
          <div className="mb-6 p-4 bg-[#D1FAE5] border border-[#4ADE80] text-[#0F172A] rounded-xl shadow-sm font-medium flex items-center gap-2">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          {/* Profile Header Section */}
          <div className="p-8 bg-gradient-to-br from-[#0F172A] to-[#0F172A]/90 relative overflow-hidden">
            <div className="absolute inset-0 opacity-10">
              <div className="absolute top-0 right-0 w-64 h-64 bg-[#4ADE80] rounded-full blur-3xl"></div>
            </div>
            
            <div className="relative flex flex-col sm:flex-row items-center sm:items-start gap-6">
              <div className="relative group">
                <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-[#4ADE80] to-[#14B8A6] flex items-center justify-center overflow-hidden ring-4 ring-[#4ADE80]/20 shadow-xl">
                  {user.avatarUrl ? (
                    <img src={user.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
                    <User className="w-14 h-14 text-[#0F172A]" />
                  )}
                </div>
                {isEditing && (
                  <div className="absolute inset-0 bg-[#0F172A]/60 rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                    <Camera className="w-6 h-6 text-white" />
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 text-center sm:text-left">
                <h2 className="text-3xl font-bold text-white mb-2">{user.name}</h2>
                <p className="text-[#D1FAE5] text-sm flex items-center gap-2 justify-center sm:justify-start">
              ...
              <button
                onClick={() => setIsEditing((s) => !s)}
                className="px-6 py-3 text-sm font-semibold bg-[#4ADE80] text-[#0F172A] hover:bg-[#14B8A6] rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl flex items-center gap-2 group"
              >
                {isEditing ? (
                  <>
                    <X className="w-4 h-4" />
          ...
          <div className="p-8">
            {isEditing ? (
              <div className="space-y-6">
                {/* Name Input */}
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-[#0F172A] ml-1">Full Name</label>
                  <input
                    type="text"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    onFocus={() => setFocusedField('name')}
                    onBlur={() => setFocusedField(null)}
                    className="w-full px-4 py-3.5 bg-[#F3F4F6] border-2 border-[#6B7280]/20 rounded-xl focus:bg-white focus:border-[#4ADE80] focus:ring-4 focus:ring-[#4ADE80]/10 transition-all duration-200 text-[#0F172A] placeholder:text-[#6B7280] outline-none"
                    placeholder="Enter your name"
                  />
                </div>
                ...
                {/* Bio Textarea */}
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-[#0F172A] ml-1">Bio</label>
                  <textarea
                    value={formData.bio}
                    onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
                    onFocus={() => setFocusedField('bio')}
                    onBlur={() => setFocusedField(null)}
                    className="w-full px-4 py-3.5 bg-[#F3F4F6] border-2 border-[#6B7280]/20 rounded-xl focus:bg-white focus:border-[#4ADE80] focus:ring-4 focus:ring-[#4ADE80]/10 transition-all duration-200 text-[#0F172A] placeholder:text-[#6B7280] outline-none resize-none"
                    rows="4"
                    placeholder="Tell us about yourself..."
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="block text-sm font-semibold text-[#0F172A] ml-1">Location</label>
                    <div className="relative">
                      <MapPin className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 transition-colors duration-200 ${
                        focusedField === 'location' ? 'text-[#4ADE80]' : 'text-[#6B7280]'
                      }`} />
                      <input
                        type="text"
                        value={formData.location}
                        ...
                        onFocus={() => setFocusedField('location')}
                        onBlur={() => setFocusedField(null)}
                        className="w-full pl-12 pr-4 py-3.5 bg-[#F3F4F6] border-2 border-[#6B7280]/20 rounded-xl focus:bg-white focus:border-[#4ADE80] focus:ring-4 focus:ring-[#4ADE80]/10 transition-all duration-200 text-[#0F172A] placeholder:text-[#6B7280] outline-none"
                        placeholder="City, Country"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-semibold text-[#0F172A] ml-1">Language</label>
                    <div className="relative">
                      <Globe className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 transition-colors duration-200 ${
                        focusedField === 'language' ? 'text-[#4ADE80]' : 'text-[#6B7280]'
                      }`} />
                      <select
                        value={formData.preferredLanguage}
                        onChange={(e) => setFormData({ ...formData, preferredLanguage: e.target.value })}
                        onFocus={() => setFocusedField('language')}
                        onBlur={() => setFocusedField(null)}
                        className="w-full pl-12 pr-4 py-3.5 bg-[#F3F4F6] border-2 border-[#6B7280]/20 rounded-xl focus:bg-white focus:border-[#4ADE80] focus:ring-4 focus:ring-[#4ADE80]/10 transition-all duration-200 text-[#0F172A] outline-none appearance-none"
                      >
                        <option value="en">English</option>
                        <option value="es">Spanish</option>
                        <option value="fr">French</option>
                ...
                {/* Avatar URL */}
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-[#0F172A] ml-1">Avatar URL</label>
                  <input
                    type="url"
                    value={formData.avatarUrl}
                    onChange={(e) => setFormData({ ...formData, avatarUrl: e.target.value })}
                ...
                <button
                  ...
                  onClick={handleUpdate}
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-[#4ADE80] to-[#14B8A6] text-[#0F172A] py-4 rounded-xl font-semibold hover:from-[#14B8A6] hover:to-[#4ADE80] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0"
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
              ...
              <div className="space-y-8">
                ...
                {user.bio && (
                  <div>
                    <h3 className="text-xs font-bold text-[#6B7280] uppercase tracking-wider mb-3">About</h3>
                    <p className="text-[#0F172A] leading-relaxed text-base">{user.bio}</p>
                  </div>
                )}

                {/* Details Grid */}
                <div>
                  <h3 className="text-xs font-bold text-[#6B7280] uppercase tracking-wider mb-4">Details</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {user.location && (
                      <div className="flex items-center gap-4 p-4 bg-[#D1FAE5] rounded-xl border border-[#4ADE80]/30 hover:shadow-md transition-shadow">
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#4ADE80] to-[#14B8A6] flex items-center justify-center flex-shrink-0 shadow-lg">
                          <MapPin className="w-5 h-5 text-[#0F172A]" />
                        </div>
                        <div>
                          <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wide">Location</p>
                          <p className="text-[#0F172A] font-medium">{user.location}</p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-4 p-4 bg-[#D1FAE5] rounded-xl border border-[#4ADE80]/30 hover:shadow-md transition-shadow">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wide">Language</p>
                        <p className="text-[#0F172A] font-medium">{user.preferredLanguage?.toUpperCase() || 'EN'}</p>
                      </div>
                    </div>
                  </div>
                </div>
